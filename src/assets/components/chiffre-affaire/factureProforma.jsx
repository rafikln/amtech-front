...
const CashRegisterDashboard = () => {
  ...
  const fetchCashRegisters = async () => {
    try {
      // Calculate totals from filtered factures
      let especeTotal = 0;
      let ccpTotal = 0;
      let yalidineTotal = 0;
  
      filteredFactures.forEach(facture => {
        const amount = parseFloat(facture.prix_total);
        const isRetour = facture.status === 'retour';
        
        // Calculate payment method totals
        facture.payment_methods.forEach(payment => {
          const paymentAmount = payment.installments.reduce(
            (sum, inst) => sum + parseFloat(inst.amount),
            0
          );
  
          if (payment.method === 'espece' || payment.method === 'cash') {
            isRetour ? especeTotal -= paymentAmount : especeTotal += paymentAmount;
          }
          if (payment.method === 'ccp') {
            isRetour ? ccpTotal -= paymentAmount : ccpTotal += paymentAmount;
          }
        });
  
        // Calculate yalidine delivery total
        if (facture.delivery?.provider === 'yalidine') {
          isRetour ? yalidineTotal -= amount : yalidineTotal += amount;
        }
      });
      ...
      console.error(err);
      toast.error("Erreur lors du calcul des totaux des caisses");
    }
  };
  ...
  const calculateTotals = (filtered) => {
    ...
    filtered.forEach(facture => {
      const amount = parseFloat(facture.prix_total);
      const isRetour = facture.status === 'retour';
      const hasCashEspece = facture.payment_methods.some(p => ['cash', 'espece'].includes(p.method));
      const isYalidinePending = facture.status === 'pending' && facture.delivery?.provider === 'yalidine';

      if (facture.status === 'valider') {
        calculatedTotal += amount;
      } else if (facture.status === 'pending' && hasCashEspece && !isYalidinePending) {
        calculatedTotal += amount;
      } else if (isRetour && ['cash', 'ccp'].some(m => facture.payment_methods.some(p => p.method === m))) {
        calculatedTotal -= amount;
      }
    });
  ...
  const applyFilters = (data) => {
    let filtered = data.filter(facture => {
      const isYalidinePending = facture.status === 'pending' && facture.delivery?.provider === 'yalidine';
      const isRetourYalidine = facture.status === 'retour' && facture.delivery?.provider === 'yalidine';
      const hasValidPayment = ['cash', 'espece', 'ccp'].some(m => 
        facture.payment_methods.some(p => p.method === m)
      );

      if (isYalidinePending || isRetourYalidine) return false;
      if (facture.status === 'retour' && !hasValidPayment) return false;
      return true;
    });
      ...
      <div className="p-4 sm:p-8 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="mb-6 flex items-center gap-4">
            <label className="text-lg font-bold text-gray-700">Filtrer les factures:</label>
            <select
              ...
              <table className="w-full bg-white rounded-lg shadow-sm">
                <thead className="bg-gray-100 sticky top-0">
                  <tr>
                    ...
                    <th className="p-3 text-left">Méthode de Paiement</th>
                ...
                <tbody>
                  {filteredFactures.map(facture => {
                    const isRetour = facture.status === 'retour';
                    const rowClass = isRetour ? 'text-red-600 bg-red-50' : '';
                    ...
                    return (
                      <tr key={facture.id} className={`border-t ${rowClass}`}>
                        <td className="p-3">{facture.id}</td>
                        <td className="p-3">
                          {parseFloat(facture.prix_total).toFixed(2)} DA
                          {isRetour && <span className="ml-2 text-xs">(Retour)</span>}
                        </td>
                        <td className="p-3">
                          {facture.payment_methods
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            <div className="text-xl font-bold text-gray-800">
              Total des Factures: {generalTotal.toLocaleString('fr-DZ', {minimumFractionDigits: 2, maximumFractionDigits: 2})} DA
            </div>
          </div>
        </div>
      </div>
